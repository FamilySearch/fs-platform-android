...
*/
  ...
  public static <Q extends Enum<Q>> Q fromURIValue(String uriValue, Class<Q> clazz, String defaultBaseUri) {
    ...
    String ns = getNamespaceString( clazz );
    Field unknown = null;
    Field[] fields = clazz.getDeclaredFields();
    for (Field field : fields) {
      if (field.isEnumConstant()) {
...
*/
  ...
  /**
   * prior to android v4.3 package annotations aren't supported. so we are going to use
   * that code here.
   * @param pkg
   ...
   * @return
   */
  private static <A extends Annotation> A getAnnotation( Package pkg, Class<A> annotationClass ) {
    try {
      Class<?> c = Class.forName(pkg.getName() + ".package-info");
